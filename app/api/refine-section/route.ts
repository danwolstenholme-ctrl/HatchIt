...
// =============================================================================
// THE REFINER - Claude Sonnet 4.5 with Extended Thinking
// Handles:
// 1. Code refinement (polish, fix, user-directed changes)
// 2. Conversational help (design suggestions, troubleshooting)
// =============================================================================
...
const HELP_SYSTEM_PROMPT = `You are an AI assistant inside HatchIt, a website builder. You help users build websites using AI-generated React/Tailwind code.

IMPORTANT: You are embedded IN the HatchIt builder. Users are currently building a website.
...
- "Preview not showing" → Usually means section is still generating. Wait for the checkmark.
- "Black screen" → Try regenerating with a simpler prompt.
- "Section looks wrong" → Type what to change in the refine bar below the preview.
- "Want to change text" → Click Edit mode (pencil icon), then double-click any text.
- "How to deploy" → Click Ship button (top right) → Deploy, GitHub, or Download.

DON'T:
- Give generic React/Tailwind tutorials
- Suggest installing packages or editing files manually
- Provide code the user needs to copy-paste

DO:
- Keep answers short (2-3 sentences max)
- Reference HatchIt UI elements by name
...
${code}`
    ...
    // Call Claude Sonnet 4.5 with extended thinking for better refinement
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
