              ...
              <div className={`relative bg-zinc-900/80 backdrop-blur-2xl rounded-2xl border transition-colors duration-300 ${
                ...
                <div className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs text-zinc-500">
                    ...
                    />
                    <span>{prompt.trim().length < 10 ? 'Describe what you want to build' : 'Try it free — no account needed'}</span>
                  </div>
                  ...
                  <motion.button
                    ...
                    whileHover={prompt.trim().length >= 10 ? { scale: 1.02 } : {}}
                    whileTap={prompt.trim().length >= 10 ? { scale: 0.98 } : {}}
                  >
                    {/* Button shine effect */}
                    {prompt.trim().length >= 10 && (
                      <motion.div
